...
class GreedyPlacement(Placement):
    ...
    """
        ...
        for aux_req in sfc_requests:
            ...
            if self.sfc_instance_sharable:
                # Verify if there is any SFC Instance that can handle the SFC_Requested
                ...
                # If the SFC_Request already had an SFC_Instance that can handle it, thus, continue to the next
                # SFC_Request
            ...
            while edges:
                ...
                for edge in edges:
                    ...
                    node_resources = self.environment.get_node_available_resource(self.vnf_instances, node_name)

                    if ant_node_resources == "" or int(node_resources['cpu']) > int(ant_node_resources['cpu']):
                        ant_node_resources = node_resources
                ...
                for edge in edges:
                    aux_link = edge[2]
                    bw_available = self.environment.calc_bw_available_link(aux_link['link_name'])
